...
import (
	...
	"github.com/AbderrahimBouhdida/chi/share/tunnel"
	"golang.org/x/crypto/ssh"
	"golang.org/x/sync/errgroup"
)

// handleClientHandler is the main http websocket handler for the chisel srv
func (s *Server) handleClientHandler(w http.ResponseWriter, r *http.Request) {
	...
	if upgrade == "websocket" {
		...
		//print into srv logs and silently fall-through
		s.Infof("ignored cl connection using protocol '%s', expected '%s'",
...
func (s *Server) handleWebsocket(w http.ResponseWriter, req *http.Request) {
	...
	// chisel srv handshake (reverse of cl handshake)
	...
	//print if cl and srv  versions dont match
	if c.Version != chshare.BuildVersion {
		v := c.Version
		if v == "" {
			v = "<unknown>"
		}
		l.Infof("Client version (%s) differs from srv version (%s)",
	...
	for _, r := range c.Remotes {
		...
		if r.Reverse && !s.config.Reverse {
			...
			failed(s.Errorf("Reverse port forwaring not enabled on srv"))
			return
		}
	...
	tunnel := tunnel.New(tunnel.Config{
		...
		Outbound:  true, //srv always accepts outbound
