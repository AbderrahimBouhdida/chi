...
// NewServer creates and returns a new chisel srv
func NewServer(c *Config) (*Server, error) {
	server := &Server{
		config:     c,
		httpServer: cnet.NewHTTPServer(),
		Logger:     cio.NewLogger("srv"),
	...
	server.sshConfig = &ssh.ServerConfig{
		ServerVersion:    "SSH-" + chshare.ProtocolVersion + "-srv",
...
// Start is responsible for kicking off the http srv
func (s *Server) Start(host, port string) error {
	return s.StartContext(context.Background(), host, port)
}

// StartContext is responsible for kicking off the http srv,
...
// Wait waits for the http srv to close
func (s *Server) Wait() error {
	return s.httpServer.Wait()
}

// Close forcibly closes the http srv
func (s *Server) Close() error {
	return s.httpServer.Close()
}

// GetFingerprint is used to access the srv fingerprint
...
// AddUser adds a new user into the srv user index
...
// DeleteUser removes a user from the srv user index
func (s *Server) DeleteUser(user string) {
	s.users.Del(user)
}

// ResetUsers in the srv user index.
