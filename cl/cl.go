...
func NewClient(c *Config) (*Client, error) {
	...
	if u.Scheme == "wss" {
		...
		if c.TLS.Cert != "" && c.TLS.Key != "" {
			...
			if err != nil {
				return nil, fmt.Errorf("Error loading cl cert and key pair: %v", err)
			}
			tc.Certificates = []tls.Certificate{c}
		} else if c.TLS.Cert != "" || c.TLS.Key != "" {
			return nil, fmt.Errorf("Please specify cl BOTH cert and key")
		}
		client.tlsConfig = tc
	}
	...
	client.sshConfig = &ssh.ClientConfig{
		...
		ClientVersion:   "SSH-" + chshare.ProtocolVersion + "-cl",
	...
	client.tunnel = tunnel.New(tunnel.Config{
		...
		Inbound:   true, //cl always accepts inbound
		Outbound:  hasReverse,
...
func (c *Client) setProxy(u *url.URL, d *websocket.Dialer) error {
	...
	if u.Scheme != "socks" && u.Scheme != "socks5h" {
		return fmt.Errorf(
			" type: %s:// (only socks5h:// or socks:// is supported)",
			u.Scheme,
		)
	}
	var auth *proxy.Auth
