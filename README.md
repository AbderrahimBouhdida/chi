...
```sh
$ chisel srv --port $PORT --proxy http://example.com
...
```sh
$ chisel cl https://chisel-demo.herokuapp.com 3000
# connects to chisel srv at https://chisel-demo.herokuapp.com,
# tunnels your localhost:3000 to the srv's localhost:3000
```
...
<!--tmpl,code=plain:echo "$ chisel srv --help" && go run main.go srv --help | cat | sed 's#0.0.0-src (go1\..*)#X.Y.Z#' -->
``` plain 
$ chisel server --help
...
<!--tmpl,code=plain:echo "$ chisel cl --help" && go run main.go cl --help | sed 's#0.0.0-src (go1\..*)#X.Y.Z#' -->
``` plain 
$ chisel client --help
...
```sh
chisel cl --fingerprint 'rHb55mcxf6vSckL2AezFV09rLs7pfPpavVu++MF7AhQ=' <srv-address>:9312 socks
```
