...
// Both chisel cl and srv are Tunnels.
// chisel cl has a single set of remotes, whereas
// chisel srv has multiple sets of remotes (one set per cl).
...
func New(c Config) *Tunnel {
	...
	//setup socks srv (not listening on any port!)
	extra := ""
	if c.Socks {
